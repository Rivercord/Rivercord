...
export async function update() {
    ...
    const res = await Unwrap(RivercordNative.updater.update());

    if (res) {
        isOutdated = false;
        if (!await Unwrap(RivercordNative.updater.rebuild()))
...
export const getRepo = () => Unwrap(RivercordNative.updater.getRepo());
