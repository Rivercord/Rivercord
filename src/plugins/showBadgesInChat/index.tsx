...
function CheckBadge({ badge, author }: { badge: string; author: any; }): JSX.Element | null {
    switch (badge) {
        ...
        case "RivercordDonor":
            return (
                <span style={{ order: settings.store.RivercordDonorPosition }}>
        ...
        case "RivercordContributer":
            return isPluginDev(author.id) ? (
                <span style={{ order: settings.store.RivercordContributorPosition }}>
                    <RoleIconComponent
                        className={roleIconClassName}
                        name={"Rivercord Contributor"}
                        size={20}
...
function ChatBadges({ author }: any) {
    return (
        <span style={{ display: "inline-flex", marginLeft: 2, verticalAlign: "top" }}>
            ...
            {settings.store.showRivercordDonor && <CheckBadge badge={"RivercordDonor"} author={author} />}
            {settings.store.showRivercordContributor && <CheckBadge badge={"RivercordContributer"} author={author} />}
...
export default definePlugin({
    ...
    start: () => {
        ...
        addDecoration("rc-show-badges-in-chat", props => <ChatBadges author={props.message?.author} />);
    },
    stop: () => {
        removeDecoration("rc-show-badges-in-chat");
    }
});
