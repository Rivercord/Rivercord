...
export function SimpleTextInput({ modalProps, onSelect, placeholder, info }: SimpleTextInputProps) {
    ...
    return (
        // @ts-ignore
        <ModalRoot className="rc-command-palette-simple-text" {...modalProps} size={ModalSize.DYNAMIC} onKeyDown={handleKeyDown}>
            <TextInput
                value={inputValue}
            ...
            />
            {info && <div className="rc-command-palette-textinfo">{info}</div>}
        </ModalRoot>
    );
}
